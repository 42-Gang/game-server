...
describe('WaitingQueueCache', () => {
  ...
  it('addToQueue: 유저를 큐에 추가한다', async () => {
    await cache.addToQueue(123);
    expect(redisMock.rpush).toHaveBeenCalledWith('writing-queue:4', 123);
  ...
  it('getQueueLength: 큐 길이를 반환한다', async () => {
    ...
    const length = await cache.getQueueLength();
  ...
  it('canStartGame: 큐에 충분한 인원이 있으면 true 반환', async () => {
    ...
    await expect(cache.canStartGame()).resolves.toBe(true);
  });

  it('canStartGame: 큐에 인원이 부족하면 false 반환', async () => {
    redisMock.llen.mockResolvedValue(2);
    await expect(cache.canStartGame()).resolves.toBe(false);
  });
  ...
  it('popForGame: 충분한 인원이 있으면 유저를 꺼내 반환', async () => {
    ...
    const users = await cache.popForGame();
  ...
  it('popForGame: 인원이 부족하면 에러 발생', async () => {
    redisMock.llen.mockResolvedValue(2);
    await expect(cache.popForGame()).rejects.toThrow(
  ...
  it('popForGame: lpop에서 null 반환 시 에러 발생', async () => {
    ...
    await expect(cache.popForGame()).rejects.toThrow('No more users in the queue');
  });
});
